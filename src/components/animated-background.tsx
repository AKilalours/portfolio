...
const AnimatedBackground = () => {
  ...
  const fireSkillEvent = (skill: Skill | null) => {
    window.dispatchEvent(new CustomEvent("spline-skill-hover", {
      detail: skill ? { name: skill.name } : { name: null }
    }));
  };

  const findSkillFromName = (name: string): Skill | null => {
    // Direct match
    const direct = SKILLS[name as SkillNames];
    if (direct) return direct;
    // Check all skills - name might be a child object, match by skill.name
  ...
  const handleMouseHover = (e: SplineEvent) => {
    ...
    if (name === "body" || name === "platform" || name === "keycap" || 
        name === "legend" || name === "keyboard" || name.startsWith("keyca")) {
      if (selectedSkillRef.current) {
        playReleaseSound();
        setSelectedSkill(null);
        selectedSkillRef.current = null;
        fireSkillEvent(null);
        try { splineApp.setVariable("heading", ""); } catch {}
        try { splineApp.setVariable("desc", ""); } catch {}
      }
      return;
    }

    if (selectedSkillRef.current?.name === name) return;

    const skill = findSkillFromName(name);
    if (skill) {
      if (selectedSkillRef.current) playReleaseSound();
      playPressSound();
      setSelectedSkill(skill);
      selectedSkillRef.current = skill;
      fireSkillEvent(skill);
      try { splineApp.setVariable("heading", skill.label); } catch {}
      try { splineApp.setVariable("desc", skill.shortDescription); } catch {}
    }
  };
  ...
  const handleSplineInteractions = () => {
    ...
    splineApp.addEventListener("keyDown", (e) => {
      ...
      const name = e.target.name;
      const direct = SKILLS[name as SkillNames];
      const skill = direct || Object.values(SKILLS).find(s => 
        s.name === name || name.toLowerCase().includes(s.name.toLowerCase())
      );
      if (skill) {
        playPressSound();
        setSelectedSkill(skill);
